...
class Polyssifier():
    ...
    def __init__(self, data, label, do_regress=DEFAULT_do_regress,
        ...
        self.data = data
        self.label = label
        self.do_regress = do_regress
        self.do_classify = not do_regress
    ...
    def run(self):
        ...
        logger.info("PATH: %s" % self.project_name)
        ...
        if self.save:
            logger.info('Saving...')
            if self.do_classify:
                np.save(os.path.join(self.project_path,
        ...
        # Print scores
        if self.verbose:
            logger.info(self.scores.astype('float').describe().transpose()
                  [['mean', 'std', 'min', 'max']])
    ...
    def fit_clf(self, args, clf_name, val, n_fold, project_name, save, scoring):
        ...
        start = time.time()
        if 0: # save and os.path.isfile(file_name):
